...
void FollowsExtractorVisitor::visitStatementList(StatementList *statementList) {
  ...
  for (StmtNum i = 0; i < statementList->statements.size(); i++) {
    ...
    writeApi->setFollowsT(statementList->statements[i]->statementNumber,
                          followsTSet);
    followsTSet.clear();
  }
}
...
void FollowsExtractorVisitor::visitPrintStatement(
    PrintStatement *printStatement) {}
void FollowsExtractorVisitor::visitAssignStatement(
...
void FollowsExtractorVisitor::visitWhileStatement(
    WhileStatement *whileStatement) {}
void FollowsExtractorVisitor::visitExpression(Expression *variable) {}
void FollowsExtractorVisitor::visitConditionalExpression(
