...
std::unique_ptr<Program> Parser::parseProgram(std::deque<Token> tokens) {
  // Rule: procedure+
  currStatementNumber =
      1; // Reset the statement number, upon parsing a new program
...
std::unique_ptr<Procedure> Parser::parseProcedure(std::deque<Token> &tokens) {
  ...
  if (!tokens.front().isType(TokenType::NAME) ||
      !tokens.front().hasValue(AppConstants::PROCEDURE)) {
    throw SyntaxErrorException("Expected 'procedure' keyword, but got -> " +
                               tokens.front().value);
  }
  tokens.pop_front();

  if (!tokens.front().isType(TokenType::NAME)) {
    throw SyntaxErrorException("Expected valid 'proc_name', but got -> " +
                               tokens.front().value);
  }
  ...
  if (!tokens.front().isType(TokenType::LEFT_BRACE)) {
    throw SyntaxErrorException("Expected '{', but got -> " +
                               tokens.front().value);
  }
  tokens.pop_front();
...
Parser::parseStatementList(std::deque<Token> &tokens) {
  // Rule: stmt+
  std::vector<std::unique_ptr<Statement>> statements;

  while (!tokens.front().isType(TokenType::RIGHT_BRACE) &&
         !tokens.front().isType(
             TokenType::ENDOFFILE)) { // Reached end of statementList
...
std::unique_ptr<Statement> Parser::parseStatement(std::deque<Token> &tokens) {
  ...
  if (tokens.front().isType(TokenType::NAME)) {
    if (tokens.at(1).isType(TokenType::ASSIGN)) { // Assign Statement
      ...
      throw SyntaxErrorException("Unexpected token when parsing statement -> " +
                                 tokens.front().value);
    }
  } else {
    throw SyntaxErrorException("Unexpected token when parsing statement -> " +
...
std::unique_ptr<ReadStatement>
Parser::parseReadStatement(std::deque<Token> &tokens) {
  // Rule: 'read' var_name';'
  tokens.pop_front(); // Pop 'read'

  if (!tokens.front().isType(TokenType::NAME)) {
    throw SyntaxErrorException(
        "Expected var_name in read statement, but got -> " +
        tokens.front().value);
  }
  ...
  if (!tokens.front().isType(TokenType::SEMICOLON)) {
    throw SyntaxErrorException(
        "Expected ; at end of read statement, but got -> " +
        tokens.front().value);
  }
...
Parser::parsePrintStatement(std::deque<Token> &tokens) {
  // Rule: 'print' var_name';'
  tokens.pop_front(); // Pop 'print'

  if (!tokens.front().isType(TokenType::NAME)) {
    throw SyntaxErrorException(
        "Expected var_name in print statement, but got -> " +
        tokens.front().value);
  }
  Ent varName = tokens.front().value;
  tokens.pop_front(); // Pop var_name
  ...
  if (!tokens.front().isType(TokenType::SEMICOLON)) {
    throw SyntaxErrorException(
        "Expected ; at end of print statement, but got -> " +
        tokens.front().value);
  }
  tokens.pop_front(); // Pop ;
...
std::unique_ptr<CallStatement>
Parser::parseCallStatement(std::deque<Token> &tokens) {
  // Rule: 'call' proc_name';'
  tokens.pop_front(); // Pop 'call'

  if (!tokens.front().isType(TokenType::NAME)) {
    throw SyntaxErrorException(
        "Expected proc_name in call statement, but got -> " +
        tokens.front().value);
  }
  ...
  if (!tokens.front().isType(TokenType::SEMICOLON)) {
    throw SyntaxErrorException(
        "Expected ; at end of call statement, but got -> " +
        tokens.front().value);
  }
  tokens.pop_front(); // Pop ;
...
Parser::parseWhileStatement(std::deque<Token> &tokens) {
  // Rule: 'while' '(' cond_expr ')' '{' stmtLst '}'
  StmtNum stmtNum =
      currStatementNumber; // Need to note down the statement number, as the
  ...
  if (!tokens.front().isType(TokenType::LEFT_PARENTHESIS)) {
    throw SyntaxErrorException("Expected '(' in while statement, but got -> " +
                               tokens.front().value);
  }
  tokens.pop_front(); // Pop '('
  ...
  return std::make_unique<WhileStatement>(stmtNum, std::move(condExpr),
                                          std::move(stmtList));
}

std::unique_ptr<IfStatement>
Parser::parseIfStatement(std::deque<Token> &tokens) {
  ...
  if (!tokens.front().isType(TokenType::LEFT_PARENTHESIS)) {
    throw SyntaxErrorException("Expected '(' in if statement, but got -> " +
                               tokens.front().value);
  }
  tokens.pop_front(); // Pop '('
  ...
  if (!tokens.front().isType(TokenType::NAME) ||
      !tokens.front().hasValue(AppConstants::THEN)) {
    throw SyntaxErrorException("Expected 'then' in if statement, but got -> " +
                               tokens.front().value);
  }
  ...
  if (!tokens.front().isType(TokenType::NAME) ||
      !tokens.front().hasValue(AppConstants::ELSE)) {
    throw SyntaxErrorException("Expected 'else' in if statement, but got -> " +
                               tokens.front().value);
  }
  ...
  return std::make_unique<IfStatement>(stmtNum, std::move(condExpr),
                                       std::move(thenStmtList),
                                       std::move(elseStmtList));
}

std::unique_ptr<AssignStatement>
Parser::parseAssignStatement(std::deque<Token> &tokens) {
  // Rule: var_name '=' expr ';'
  ...
  if (!tokens.front().isType(TokenType::SEMICOLON)) {
    throw SyntaxErrorException(
        "Expected ';' at end of assign statement, but got -> " +
        tokens.front().value);
  }
  tokens.pop_front(); // Pop ;

  return std::make_unique<AssignStatement>(currStatementNumber++, varName,
                                           std::move(expr));
}

std::unique_ptr<ConditionalExpression>
Parser::parseConditionalExpression(std::deque<Token> &tokens) {
  ...
  if (tokens.front().isType(TokenType::NOT)) { // '!' '(' path
    ...
    if (!tokens.front().isType(TokenType::LEFT_PARENTHESIS)) {
      throw SyntaxErrorException(
          "Expected '(' in not conditional expression, but got -> " +
          tokens.front().value);
    }
    tokens.pop_front(); // Pop '('
  ...
  } else if (tokens.front().isType(
                 TokenType::LEFT_PARENTHESIS)) { // Binary Cond Expr Path
    ...
    if (!tokens.front().isType(TokenType::RIGHT_PARENTHESIS)) {
      throw SyntaxErrorException(
          "Expected ')' in binary conditional expression, but got -> " +
          tokens.front().value);
    }
    tokens.pop_front(); // Pop ')'
    ...
    if (!tokens.front().isType(TokenType::RIGHT_PARENTHESIS)) {
      throw SyntaxErrorException(
          "Expected ')' in conditional expression, but got -> " +
          tokens.front().value);
    }
    tokens.pop_front(); // Pop ')'

    return std::make_unique<BinaryConditionalExpression>(
        condOperator, std::move(lhs), std::move(rhs));
...
std::unique_ptr<ConditionalExpression>
Parser::parseRelationalExpression(std::deque<Token> &tokens) {
  ...
  */
  ...
  if (!tokens.front().isType(TokenType::RELATIONAL_OPR)) {
    throw SyntaxErrorException(
        "Expected a comparator in relational expression, but got -> " +
        tokens.front().value);
  }
  ...
  return std::make_unique<RelationalExpression>(relationalOperator,
                                                std::move(lhs), std::move(rhs));
}

std::unique_ptr<Expression>
Parser::parseRelationalFactor(std::deque<Token> &tokens) {
  // Rule: var_name | const_value | expr
  // From this, we know that we can parse tokens.front() as either a constant or
  // variable.
  if (tokens.at(1).isType(TokenType::RELATIONAL_OPR) ||
      tokens.at(1).isType(TokenType::RIGHT_PARENTHESIS)) {
...
std::unique_ptr<Expression> Parser::parseExpression(std::deque<Token> &tokens) {
  ...
  if (tokens.front().isType(TokenType::EXPR_ARITH_OPR)) {
    ...
    return std::make_unique<MathExpression>(mathOperator, std::move(lhs),
                                            std::move(rhs));
...
std::unique_ptr<Expression> Parser::parseFactor(std::deque<Token> &tokens) {
  ...
  if (tokens.front().isType(TokenType::INTEGER)) {
    ...
    if (!tokens.front().isType(TokenType::RIGHT_PARENTHESIS)) {
      throw SyntaxErrorException("Expected ')' in factor, but got -> " +
                                 tokens.front().value);
    }
    tokens.pop_front(); // Pop ')'
    ...
    throw SyntaxErrorException("Parsing factor failed, got -> " +
                               tokens.front().value);
  }
}
