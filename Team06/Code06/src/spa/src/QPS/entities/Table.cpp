...
// Implementation using unorderedMap
...
vector<Parameter> Table::getIntersectingParams(Table t) {
  ...
  for(const auto& kv1: table) {
    for(const auto& headers : t.getHeaders()) {
      if (kv1.first == headers) {
        intersectingParams.push_back(headers);
        break;
      }
    }
  }
...
// original Table implementation using just vectors.
