...
bool checkIfSameCondition(std::unique_ptr<ConditionalExpression> expectedExpression,
...
TEST_CASE("Invalid Source Program") {
    ...
    SECTION("Invalid read statement variable name") {
        ...
        REQUIRE(errorMessage.find("Expected var_name in read statement") != std::string::npos);
    }
    ...
    SECTION("Missing semicolon for read statement") {
        ...
        REQUIRE(errorMessage.find("Expected ; at end of read statement") != std::string::npos);
    }
    ...
    SECTION("Invalid print statement variable name") {
        ...
        REQUIRE(errorMessage.find("Expected var_name in print statement") != std::string::npos);
    }
    ...
    SECTION("Missing semicolon for read statement") {
        ...
        REQUIRE(errorMessage.find("Expected ; at end of print statement") != std::string::npos);
    }
    ...
    SECTION("Invalid call statement procedure name") {
        ...
        REQUIRE(errorMessage.find("Expected proc_name in call statement") != std::string::npos);
    }
    ...
    SECTION("Missing semicolon for call statement") {
        ...
        REQUIRE(errorMessage.find("Expected ; at end of call statement") != std::string::npos);
    }
    ...
    SECTION("Missing ( in while") {
        ...
        REQUIRE(errorMessage.find("Expected '(' in while statement") != std::string::npos);
    }
    ...
    SECTION("Missing ( in if") {
        ...
        REQUIRE(errorMessage.find("Expected '(' in if statement") != std::string::npos);
    }
    ...
    SECTION("Missing ( for not conditional expression") {
        ...
        REQUIRE(errorMessage.find("Expected '(' in not conditional expression") != std::string::npos);
    }
    ...
    SECTION("Missing ) in binary conditional") {
        ...
        REQUIRE(errorMessage.find("Expected ')' in binary conditional expression") != std::string::npos);
    }
...
bool checkIfSameCondition(std::unique_ptr<ConditionalExpression> expectedExpression,
    ...
    if (CAST_TO(NotConditionalExpression, expected) && CAST_TO(NotConditionalExpression, actual)) {
        ...
        bool isSameLeftConditional =
            checkIfSameCondition(std::move(expectedCondition->lhs), std::move(actualCondition->lhs));
        bool isSameRightConditional =
            checkIfSameCondition(std::move(expectedCondition->rhs), std::move(actualCondition->rhs));
        ...
        bool isSameLeftExpression =
            checkIfSameExpression(std::move(expectedCondition->lhs), std::move(actualCondition->lhs));
        bool isSameRightExpression =
            checkIfSameExpression(std::move(expectedCondition->rhs), std::move(actualCondition->rhs));
...
bool checkIfSameStatement(std::unique_ptr<Statement> expectedStatement, std::unique_ptr<Statement> actualStatement) {
    ...
    if (CAST_TO(PrintStatement, expected) && CAST_TO(PrintStatement, actual)) {
        ...
        bool isSameStatementNumber =
            (expectedAssignStatement->statementNumber == actualAssignStatement->statementNumber);
        bool isSameVariable = (expectedAssignStatement->varName == actualAssignStatement->varName);
        bool isSameExpression =
            checkIfSameExpression(std::move(expectedAssignStatement->expr), std::move(actualAssignStatement->expr));
        ...
        bool isSameCondition =
            checkIfSameCondition(std::move(expectedIfObject->condExpr), std::move(actualIfObject->condExpr));
        bool isSameThenStatements = checkIfSameStatementList(std::move(expectedIfObject->thenStmtList),
                                                             std::move(actualIfObject->thenStmtList));
        bool isSameElseStatements = checkIfSameStatementList(std::move(expectedIfObject->elseStmtList),
        ...
        bool isSameCondition =
            checkIfSameCondition(std::move(expectedWhileObject->condExpr), std::move(actualWhileObject->condExpr));
        bool isSameStatements =
            checkIfSameStatementList(std::move(expectedWhileObject->stmtList), std::move(actualWhileObject->stmtList));
