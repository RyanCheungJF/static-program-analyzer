...
TEST_CASE("findRelationship(shared_ptr<Relationship> rs), Modifies / Uses S") {
  ...
  SECTION(
      "findRelationship(shared_ptr<Relationship> rs): non-empty storage, negative case, LHS: StmtNum, RHS: non-exist") {
    // Modifies (1, "nonExist")
  ...
  SECTION(
      "findRelationship(shared_ptr<Relationship> rs): non-empty storage, positive case, LHS: StmtNum, RHS: wildcard") {
    // Modifies (1, _)
  ...
  SECTION(
      "findRelationship(shared_ptr<Relationship> rs): non-empty storage, positive case, LHS: Stmt, RHS: non-wildcard") {
    // Modifies (s, "a")
