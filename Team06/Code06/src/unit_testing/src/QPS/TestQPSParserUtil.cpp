...
TEST_CASE("FindSuchThat / such that present  / original wordlist not modified "
  ...
  vector<string> wordList = {"random", "string", "/asd", "such",
                             "that",   "asd",    ",ksl"};
...
TEST_CASE("FindSuchThat / multiple such that present / returns index of all "
          "such that") {
  vector<string> wordList = {"such", "such", "that", "such",
                             "that", "that", ",such"};
...
TEST_CASE("FindSuchThat / only such, no that present / return -1") {
  vector<string> wordList = {"such", "such", "asdf", "such",
                             "qwe",  "123",  ",asd"};
  vector<int> expected;
  vector<int> result = findSuchThat(wordList);
...
TEST_CASE("FindSuchThat / only that, no such present / return -1") {
  vector<string> wordList = {"that", "that", "asdf", "that",
                             "qwe",  "123",  ",asd"};
  vector<int> expected;
  vector<int> result = findSuchThat(wordList);
...
TEST_CASE("FindSuchThat / both such that present, but not consecutive in that "
          "order / return -1") {
  vector<string> wordList = {"that", "such", "asdf", "that",
                             "such", "123,", "that"};
  vector<int> expected;
  vector<int> result = findSuchThat(wordList);
...
TEST_CASE("FindSuchThat / neither such nor that present / return -1") {
  vector<string> wordList = {"qwee", "asd", "asdf", "vxx",
                             "qwe",  "123", ",asd"};
  vector<int> expected;
  vector<int> result = findSuchThat(wordList);
...
TEST_CASE("extractParameters / one clause with brackets in third parameter / "
  ...
  tuple<string, string, string, string> expectedTuple("a", "sd",
                                                      "\"(x*y)+(z*y)\"", "");
  tuple<string, string, string, string> output = extractParameters(input);
  REQUIRE(expectedTuple == output);
}
...
TEST_CASE(
    "extractParameters / one clause with no closing bracket / throws error") {
  string input = "a(sd,\"x*y\"";
...
TEST_CASE(
    "extractParameters / one clause with no opening bracket / throws error") {
  string input = "asd,\"x*y\")";
  REQUIRE_THROWS(extractParameters(input));
}

TEST_CASE(
    "extractParameters / one clause with not proper format / throws error") {
  string input = "asd,\"x*y\"";
  REQUIRE_THROWS(extractParameters(input));
}
