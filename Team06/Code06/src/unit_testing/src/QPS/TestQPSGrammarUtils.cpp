...
TEST_CASE(
    "IsName / Name is combination of upper and lower case / return true") {
  string name = "APpLe";
  REQUIRE(isName(name));
}
...
TEST_CASE(
    "IsName / name cannot contain non letters and numbers / return false") {
  string name = "app_le";
  REQUIRE(!isName(name));
}
...
TEST_CASE(
    "IsIdent / ident is combination of upper and lower case / return true") {
  string name = "APpLe";
  REQUIRE(isIdent(name));
}

TEST_CASE(
    "IsIdent / ident is combination of letters and numbers / return true") {
  string name = "Br0Th4";
  REQUIRE(isIdent(name));
}
...
TEST_CASE(
    "IsIdent / ident cannot contain non letters and numbers / return false") {
  string name = "app_le";
  REQUIRE(!isIdent(name));
}
...
TEST_CASE(
    "isSynonym / synonym is combination of letters and numbers / return true") {
  string name = "Br0Th4";
  REQUIRE(isSynonym(name));
}
...
TEST_CASE(
    "isSelect / select with small s is NOT a select statement / return false") {
  string s = "select";
  REQUIRE(!isSelect(s));
}
...
TEST_CASE(
    "isDesignEntity / empty string is not a design entity / return false") {
  string s = "";
