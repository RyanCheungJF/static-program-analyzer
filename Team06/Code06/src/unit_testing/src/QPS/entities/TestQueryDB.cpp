...
TEST_CASE("insertTable / insertion of three tables with intersection / return "
  ...
  vector<Parameter> expectedParams = {Parameter("s1", ParameterType::STMT),
                                      ...
                                      Parameter("s2", ParameterType::STMT),
                                      Parameter("a", ParameterType::ASSIGN)};
  vector<string> aa = qdb.fetch(Parameter("s1", ParameterType::STMT));
  bool a = qdb.fetch(Parameter("s1", ParameterType::STMT)) == expectedCols[0];
  bool b = qdb.fetch(Parameter("v", ParameterType::VARIABLE)) == expectedCols[1];
  bool c = qdb.fetch(Parameter("s2", ParameterType::STMT)) == expectedCols[2];
  bool d = qdb.fetch(Parameter("a", ParameterType::ASSIGN)) == expectedCols[3];
  REQUIRE((a && b && c && d));
}
...
TEST_CASE("insertTable / third table intersects with first two tables / return "
  ...
  vector<string> aa = qdb.fetch(Parameter("s1", ParameterType::STMT));
  bool a = qdb.fetch(Parameter("s1", ParameterType::STMT)) == expectedCols[0];
  bool b = qdb.fetch(Parameter("s2", ParameterType::STMT)) == expectedCols[1];
  bool c = qdb.fetch(Parameter("x", ParameterType::VARIABLE)) == expectedCols[2];
  bool d = qdb.fetch(Parameter("y", ParameterType::VARIABLE)) == expectedCols[3];
  REQUIRE((a && b && c && d));
}
...
TEST_CASE("insertTable / insert tables with empty content / tables can still "
  ...
  qdb.insertTable(t2);
  qdb.insertTable(t3);
  bool a = qdb.fetch(Parameter("s1", ParameterType::STMT)).empty();
  bool b = qdb.fetch(Parameter("s2", ParameterType::STMT)).empty();
  bool c = qdb.fetch(Parameter("x", ParameterType::VARIABLE)).empty();
  bool d = qdb.fetch(Parameter("y", ParameterType::VARIABLE)).empty();
  REQUIRE((a && b && c && d));
}