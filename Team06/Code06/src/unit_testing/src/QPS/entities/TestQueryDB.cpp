...
TEST_CASE("insertTable / insertion of two tables with intersection / return "
  ...
  REQUIRE(((finalContent[0] == "1" && (finalContent[1] == "2")) &&
           finalContent.size() == 2));
}
...
TEST_CASE("insertTable / insertion of three tables with intersection / return "
  ...
  vector<Parameter> params2 = {Parameter("s2", AppConstants::STMT),
                               Parameter("v", AppConstants::VARIABLE)};
  vector<Parameter> params3 = {Parameter("s2", AppConstants::STMT),
  ...
  vector<vector<string>> expectedCols = {
      {"1", "2", "3"}, {"x", "y"}, {"4", "5"}, {"99"}};
  vector<Parameter> expectedParams = {Parameter("s1", AppConstants::STMT),
                                      Parameter("v", AppConstants::VARIABLE),
                                      Parameter("s2", AppConstants::STMT),
                                      Parameter("a", AppConstants::ASSIGN)};
  bool a = qdb.fetch({Parameter("s1", AppConstants::STMT)}, readPKB) == expectedCols[0];
  bool b = qdb.fetch({Parameter("v", AppConstants::VARIABLE)}, readPKB) == expectedCols[1];
  bool c = qdb.fetch({Parameter("s2", AppConstants::STMT)}, readPKB) == expectedCols[2];
  bool d = qdb.fetch({Parameter("a", AppConstants::ASSIGN)}, readPKB) == expectedCols[3];
...
TEST_CASE("insertTable / third table intersects with first two tables / return "
  ...
  vector<vector<string>> content3 = {
      {"4", "1"}, {"5", "2"}, {"4", "3"}, {"5", "3"}};
  vector<Parameter> params1 = {Parameter("s1", AppConstants::STMT),
                               Parameter("x", AppConstants::VARIABLE)};
  vector<Parameter> params2 = {Parameter("s2", AppConstants::STMT),
                               Parameter("y", AppConstants::VARIABLE)};
  vector<Parameter> params3 = {Parameter("s2", AppConstants::STMT),
  ...
  vector<vector<string>> expectedCols = {
      {"1", "2", "3"}, {"4", "5"}, {"x", "y"}, {"a", "b"}};
  bool a = qdb.fetch({Parameter("s1", AppConstants::STMT)}, readPKB) == expectedCols[0];
  bool b = qdb.fetch({Parameter("s2", AppConstants::STMT)}, readPKB) == expectedCols[1];
  bool c = qdb.fetch({Parameter("x", AppConstants::VARIABLE)}, readPKB) == expectedCols[2];
  bool d = qdb.fetch({Parameter("y", AppConstants::VARIABLE)}, readPKB) == expectedCols[3];
  REQUIRE((a && b && c && d));
}
...
TEST_CASE("insertTable / insert tables with empty content / tables can still "
  ...
  qdb.insertTable(t2);
  qdb.insertTable(t3);
  bool a = qdb.fetch({Parameter("s1", AppConstants::STMT)},readPKB).empty();
  bool b = qdb.fetch({Parameter("s2", AppConstants::STMT)}, readPKB).empty();
  bool c = qdb.fetch({Parameter("x", AppConstants::VARIABLE)}, readPKB).empty();
  bool d = qdb.fetch({Parameter("y", AppConstants::VARIABLE)}, readPKB).empty();
  REQUIRE((a && b && c && d));
}